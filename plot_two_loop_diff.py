...
E = (np.arange(0,25)+1)*4.0 #np.array([2.0, 2.43154, 2.95619, 3.59405, 4.36953, 5.31235, 6.45859, 7.85216, 9.54642, 11.6062, 14.1105, 17.1551, 20.8567, 25.3569, 30.8282, 37.48])
...
def makeplot(location):
    ...
    t = np.array(f1["r(cm)"] - f2["r(cm)"]) / clight * 1e12
    fmatrixf = np.array(f1["fmatrixf"] - f2["fmatrixf"])
